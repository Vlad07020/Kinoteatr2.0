...
&НаКлиенте
Процедура ВыбратьФильм(Команда)
	а=1;
КонецПроцедуры
...
Процедура ОтобразитьСеансыВПланировщике()
	...
	Пока Выборка.Следующий() Цикл
		...
		НачалоСеанса = Выборка.Период + Выборка.ВремяСеанса - НачалоДня(Выборка.ВремяСеанса);
		КонецСеанса = НачалоСеанса + Выборка.Фильм.Длительность*60;
